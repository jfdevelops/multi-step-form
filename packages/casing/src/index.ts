...
export namespace casing {
  ...
  export type CapitalizeWord<S extends string> =
    S extends `${infer F}${infer R}` ? `${Uppercase<F>}${ToLower<R>}` : S;
  ...
  export const DEFAULT_CASING: types.SetDefaultString<CasingType, 'title'> =
    'title';
