...
`withOverrides`) but producing full React `MultiStepFormSchema` instances — the same objects
`createMultiStepFormSchema` already returns, with `.withForm()` / `.withContext()` /
`createComponent` available on every instance.
