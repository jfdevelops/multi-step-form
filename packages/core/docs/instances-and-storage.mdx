...
`defineMultiStepForm` is the single form-definition API starting with the `beta` pre-releases.
It supports a default single instance as well as named instances — e.g. a public/client form and
an internal/admin form that share the same steps and helper functions, or a form that shouldn't
...
```ts
instance.stepSchema.value.step1.isComplete; // live boolean
